...
//static inline void *malloc_copy(void *src, int size)
//{
...
static int av_reallocp_array(void *ptr, size_t nmemb, size_t size)
{
	void *val;

	memcpy(&val, ptr, sizeof(val));
	val = realloc(val, nmemb*size);
...
#ifdef NOALLOC_NALUNIT
	array->nalUnit[numNalus] = nal_buf;
#else
	array->nalUnit[numNalus] = malloc(nal_size);
	memcpy(array->nalUnit[numNalus], nal_buf, nal_size);
#endif
...
int hvcc_add_nal_unit(uint8_t *nal_buf, uint32_t nal_size,
...
#ifndef NOALLOC_NALUNIT	
		for (j = 0; j < hvcc->array[i].numNalus; j++)
		{
			free(hvcc->array[i].nalUnit[j]);
		}
#endif
...
static int hvcc_write(FSIOContext *pb, HEVCDecoderConfigurationRecord *hvcc)
{
	uint8_t i;
	...
	*/
	...
	fsio_w8(pb, hvcc->configurationVersion);
	...
	*/
	fsio_w8(pb, hvcc->general_profile_space << 6 |
		hvcc->general_tier_flag << 5 |
		hvcc->general_profile_idc);

	/* unsigned int(32) general_profile_compatibility_flags; */
	fsio_wb32(pb, hvcc->general_profile_compatibility_flags);

	/* unsigned int(48) general_constraint_indicator_flags; */
	fsio_wb32(pb, hvcc->general_constraint_indicator_flags >> 16);
	fsio_wb16(pb, hvcc->general_constraint_indicator_flags);

	/* unsigned int(8) general_level_idc; */
	fsio_w8(pb, hvcc->general_level_idc);

	/*
	* bit(4) reserved = �?111’b;
	* unsigned int(12) min_spatial_segmentation_idc;
	*/
	fsio_wb16(pb, hvcc->min_spatial_segmentation_idc | 0xf000);

	/*
	* bit(6) reserved = �?11111’b;
	* unsigned int(2) parallelismType;
	*/
	fsio_w8(pb, hvcc->parallelismType | 0xfc);

	/*
	* bit(6) reserved = �?11111’b;
	* unsigned int(2) chromaFormat;
	*/
	fsio_w8(pb, hvcc->chromaFormat | 0xfc);

	/*
	* bit(5) reserved = �?1111’b;
	* unsigned int(3) bitDepthLumaMinus8;
	*/
	fsio_w8(pb, hvcc->bitDepthLumaMinus8 | 0xf8);

	/*
	* bit(5) reserved = �?1111’b;
	* unsigned int(3) bitDepthChromaMinus8;
	*/
	fsio_w8(pb, hvcc->bitDepthChromaMinus8 | 0xf8);

	/* bit(16) avgFrameRate; */
	fsio_wb16(pb, hvcc->avgFrameRate);
	...
	*/
	fsio_w8(pb, hvcc->constantFrameRate << 6 |
		hvcc->numTemporalLayers << 3 |
		hvcc->temporalIdNested << 2 |
		hvcc->lengthSizeMinusOne);

	/* unsigned int(8) numOfArrays; */
	fsio_w8(pb, hvcc->numOfArrays);
	...
	for (i = 0; i < hvcc->numOfArrays; i++) {
		...
		*/
		fsio_w8(pb, hvcc->array[i].array_completeness << 7 |
			hvcc->array[i].NAL_unit_type & 0x3f);

		/* unsigned int(16) numNalus; */
		fsio_wb16(pb, hvcc->array[i].numNalus);

		for (j = 0; j < hvcc->array[i].numNalus; j++) {
			/* unsigned int(16) nalUnitLength; */
			fsio_wb16(pb, hvcc->array[i].nalUnitLength[j]);

			/* bit(8*nalUnitLength) nalUnit; */
			fsio_write(pb, hvcc->array[i].nalUnit[j],
...
static int hvcc_read(FSIOContext *pb, HEVCDecoderConfigurationRecord *hvcc)
{
	int nRet = 0;
	uint8_t i;
	uint16_t j, ps_count = 0;

	/* unsigned int(8) configurationVersion = 1; */
	hvcc->configurationVersion = fsio_r8(pb);
	...
	* unsigned int(5) general_profile_idc;
	*/
	uint8_t hvcc_general = fsio_r8(pb);
	hvcc->general_profile_space = (hvcc_general >> 6);
	hvcc->general_tier_flag = (hvcc_general >> 5) & 0x1;
	hvcc->general_profile_idc = hvcc_general & 0x1f;

	/* unsigned int(32) general_profile_compatibility_flags; */
	hvcc->general_profile_compatibility_flags = fsio_rb32(pb);

	/* unsigned int(48) general_constraint_indicator_flags; */

	hvcc->general_constraint_indicator_flags = (fsio_rb32(pb) << 16) | fsio_rb16(pb);
	/* unsigned int(8) general_level_idc; */
	hvcc->general_level_idc = fsio_r8(pb);

	/*
	* bit(4) reserved = �?111’b;
	* unsigned int(12) min_spatial_segmentation_idc;
	*/
	hvcc->min_spatial_segmentation_idc = fsio_rb16(pb)&(~0xf000);

	/*
	* bit(6) reserved = �?11111’b;
	* unsigned int(2) parallelismType;
	*/
	hvcc->parallelismType = fsio_r8(pb) & (~0xfc);

	/*
	* bit(6) reserved = �?11111’b;
	* unsigned int(2) chromaFormat;
	*/
	hvcc->chromaFormat = fsio_r8(pb) & (~0xfc);

	/*
	* bit(5) reserved = �?1111’b;
	* unsigned int(3) bitDepthLumaMinus8;
	*/
	hvcc->bitDepthLumaMinus8 = fsio_r8(pb) & (~0xf8);

	/*
	* bit(5) reserved = �?1111’b;
	* unsigned int(3) bitDepthChromaMinus8;
	*/
	hvcc->bitDepthChromaMinus8 = fsio_r8(pb) & (~0xf8);

	/* bit(16) avgFrameRate; */
	hvcc->avgFrameRate = fsio_rb16(pb);
	...
	*/
	...
	/* unsigned int(8) numOfArrays; */
	hvcc->numOfArrays = fsio_r8(pb);

	if (hvcc->numOfArrays > MAX_SHORT_TERM_RPS_COUNT)
		return ERROR_INVALIDDATA;
	hvcc->array = calloc(hvcc->numOfArrays, sizeof(HVCCNALUnitArray));
	if (!hvcc->array)
		return -1;
	...
	for (i = 0; i < hvcc->numOfArrays; i++) {
		...
		*/
		...
		hvcc->array[i].NAL_unit_type = fsio_r8(pb)/* & 0x3f*/;
		hvcc->array[i].numNalus = fsio_rb16(pb);
		if ((hvcc->array[i].NAL_unit_type == HEVC_NAL_VPS && hvcc->array[i].numNalus > MAX_VPS_COUNT) |
			(hvcc->array[i].NAL_unit_type == HEVC_NAL_SPS && hvcc->array[i].numNalus > MAX_SPS_COUNT) |
			(hvcc->array[i].NAL_unit_type == HEVC_NAL_PPS && hvcc->array[i].numNalus > MAX_PPS_COUNT)){
			nRet = -1;
			break;
		}
			

		hvcc->array[i].nalUnitLength = calloc(hvcc->array[i].numNalus,sizeof(uint16_t));
		hvcc->array[i].nalUnit = calloc(hvcc->array[i].numNalus, sizeof(uint8_t*));
		for (j = 0; j < hvcc->array[i].numNalus; j++) {
			/* unsigned int(16) nalUnitLength; */
			hvcc->array[i].nalUnitLength[j] = fsio_rb16(pb);
			hvcc->array[i].nalUnit[j] = malloc(hvcc->array[i].nalUnitLength[j]);

			/* bit(8*nalUnitLength) nalUnit; */
			fsio_read(pb, hvcc->array[i].nalUnit[j],
				hvcc->array[i].nalUnitLength[j]);
		}
	}
...
//for mp4
int DecoderConfiguration_to_Mp4hvcC(HEVCDecoderConfigurationRecord *hvcc, uint8_t *out_buf, uint32_t max_size, uint32_t *out_size)
{
	FSIOContext ioc;
	fsio_init_context(&ioc, out_buf, max_size, 0);

	return hvcc_write(&ioc, hvcc);
}
int Mp4hvcC_to_DecoderConfiguration(uint8_t *in_buf, uint32_t in_size, HEVCDecoderConfigurationRecord *hvcc)
{
	FSIOContext ioc;
	fsio_init_context(&ioc, in_buf, in_size, 0);

	return hvcc_read(&ioc, hvcc);
}