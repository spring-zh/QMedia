...
void ShaderProgram::setVertexAttribValue(const char* key, VertexAttrib::Vec2<float> value[], int count) {
    setVertexAttribValue(key, (float*)value, count*2, 2);
}
void ShaderProgram::setVertexAttribValue(const char* key, VertexAttrib::Vec3<float> value[], int count) {
    setVertexAttribValue(key, (float*)value, count*3, 3);
}
void ShaderProgram::setVertexAttribValue(const char* key, VertexAttrib::Vec4<float> value[], int count) {
    setVertexAttribValue(key, (float*)value, count*4, 4);
}
void ShaderProgram::setVertexAttribValue(const char* key, float data[], int data_size, int vec_size) {
    ...
    if (iter == attrib_storage_.end()) { //first set
        ...
        attrib.gl_binder.location = GetAttribLocation(key);
        if (enable_vbo_){
...
void ShaderProgram::setUniformValue(const char* key, int value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform1i(location, value));
}
void ShaderProgram::setUniformValue(const char* key, int value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform1iv(location, count, value));
}
void ShaderProgram::setUniformValue(const char* key, float value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform1f(location, value));
}
void ShaderProgram::setUniformValue(const char* key, float value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform1fv(location, count, value));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec2<int> value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform2i(location, value.x, value.y));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec2<int> value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform2iv(location, count, (const int*)&value));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec2<float> value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform2f(location, value.x, value.y));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec2<float> value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform2fv(location, count, (const float*)&value));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec3<int> value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform3i(location, value.x, value.y, value.z));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec3<int> value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform3iv(location, count, (const int*)&value));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec3<float> value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform3f(location, value.x, value.y, value.z));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec3<float> value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform3fv(location, count, (const float*)&value));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec4<int> value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform4i(location, value.x, value.y, value.z, value.w));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec4<int> value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform4iv(location, count, (const int*)&value));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec4<float> value) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform4f(location, value.x, value.y, value.z, value.w));
}
void ShaderProgram::setUniformValue(const char* key, Uniform::Vec4<float> value[], int count) {
    GLint location = GetUniformLocation(key);
    CHECK_GL_ERROR(glUniform4fv(location, count, (const float*)&value));
}

void ShaderProgram::setUniformValue(const char* key, Uniform::Matrix matrix) {
    GLint location = GetUniformLocation(key);
    if (matrix.level == 2) {
...
void ShaderProgram::setUniformValue(const char* key, Uniform::TextureUnit texutre_unit) {
...
void ShaderProgram::prepareTextures(const TextureStorage& texture_storage, unsigned int& texture_idx) {
...
void ShaderProgram::prepareAttribs(const AttribStorage& attrib) {
...
void ShaderProgram::setIndicesValue(Variant &value)
{
...
GLint ShaderProgram::GetUniformLocation(const char* key) {
    GLint location;
    CHECK_GL_ERROR(location = glGetUniformLocation(program_, key));
    return location;
}

GLint ShaderProgram::GetAttribLocation(const char* key) {
    GLint location;
    CHECK_GL_ERROR(location = glGetAttribLocation(program_, key));
    return location;
}
