...
const Texture2D * EffectGroup::drawEffects(int64_t duration, RenderDeviceGL* gle, const Texture2D *inputTexture){
    int idx = 0;
    const Texture2D* current = inputTexture;
    gle->saveStatus();
    if (_framebuffer == nullptr) {
        _framebuffer = std::shared_ptr<FrameBufferGL>(FrameBufferGL::createNew());
    }
    for (auto& effect : _effects) {
        if (effect->_timeRange.isContain(duration)) {
            ...
            _framebuffer->attachTexture2D(FrameBufferGL::COLOR, next);
            
            Api::Vec4f layerViewPort(0, 0, next->width(), next->height());
