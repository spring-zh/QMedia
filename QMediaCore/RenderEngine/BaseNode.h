...
using RenderEngine::Mat4;
using RenderEngine::Vec2;
using RenderEngine::Quaternion;
...
protected:
    ...
    Mat4 modelViewTransform_;
    
    Quaternion rotationQuat_;       ///rotation using quaternion, if _rotationZ_X == _rotationZ_Y, _rotationQuat = RotationZ_X * RotationY * RotationX, else _rotationQuat = RotationY * RotationX
    
    // "cache" variables are allowed to be mutable
    mutable Mat4 transform_;        ///< transform
    mutable bool transformDirty_;   ///< transform dirty flag
    mutable Mat4 inverse_;          ///< inverse transform
    mutable bool inverseDirty_;     ///< inverse transform dirty flag
    mutable Mat4* additionalTransform_ = NULL; ///< two transforms needed by additional transforms
