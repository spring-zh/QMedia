...
SceneNode::SceneNode():_flipMode(RenderEngine::Drawable2D::NONE){
...
void SceneNode::visit(RenderEngine::Scene *scene, const Mat4& parentTransform, uint32_t parentFlags)
{
...
bool SceneNode::drawEffects(int64_t duration, const RenderEngine::Scene *scene, const Mat4 & transform, const RenderEngine::Texture2D *inputTexture) {
    const RenderEngine::Texture2D *output = _effect_group.drawEffects(duration, scene->getGLEngine(), inputTexture);
    if (output) {
...
void SceneNode::addEffect(RenderEngine::EffectRef effectRef) {
    _effect_group.addEffect(effectRef);
}
void SceneNode::removeEffect(RenderEngine::EffectRef effectRef) {
...
void SceneNode::__visit(RenderEngine::Scene *scene, const Mat4& parentTransform, uint32_t parentFlags) {
    if (!visible_)
    {
        return;
    }
