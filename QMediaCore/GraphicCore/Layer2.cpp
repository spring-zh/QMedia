...
Layer2::Layer2(GraphicCore::Size size): _layerSize(size) {
}
...
void Layer2::visit(GraphicCore::Scene *scene, const Mat4& parentTransform, uint32_t parentFlags)
{
...
//    gle->setCurrentFrameBuffer(_framebuffer.get());
    fbo_texture_->use();
    Rect layerViewPort(0, 0, fbo_texture_->width(),fbo_texture_->height());
...
void Layer2::draw(GraphicCore::Scene* scene, const Mat4 & transform, uint32_t flags) {
...
void Layer2::duplicateDraw(GraphicCore::Scene* scene, const Mat4 & transform, SceneNode* displayNode) {
    if (fbo_texture_ && _textureDrawer) {
