...
/// Output samples from beginning of the sample buffer. Copies requested samples to
/// output buffer and removes them from the sample buffer. If there are less than
/// 'fixedSamples' samples in the buffer, returns fasle.
///
/// \return true if there is enough data,else fase
bool SoundTouch::receiveFixedSamples(SAMPLETYPE *outBuffer, uint fixedSamples) {
    bool ret = FIFOProcessor::receiveFixedSamples(outBuffer, fixedSamples);
    if (ret) {
        samplesOutput += (long) fixedSamples;
    }
    return ret;
}
...
///get audio channels processed now
int SoundTouch::getChannels() {
    return channels;
}
///get audio sampleRate processed now
int SoundTouch::getSampleRate() {
    return sampleRate;
}
