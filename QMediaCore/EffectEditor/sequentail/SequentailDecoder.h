...
public:
    ...
    SequentailDecoder(MediaType media_type, CallBack* callback, std::string name = "");
	virtual ~SequentailDecoder() {}
    
    void SetMaxOutputFrameCount(int max_count) { max_count_ = max_count; }

    virtual std::future<bool> StartMedia(std::vector<SequentailSlice>& slices, int64_t time_ms = 0);
...
protected:
     ...
     */
    std::vector<SequentailSlice> slices_;
