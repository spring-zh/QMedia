...
SequentailDecoder::SequentailDecoder(MediaType media_type, CallBack* callback, std::string name):
...
void SequentailDecoder::ReadDecode() {
    ...
    if(ret < 0 ) {
        callback_->OnSequentailError(ret);
    } else {
...
std::future<bool> SequentailDecoder::StartMedia(std::vector<SequentailSlice>& slices, int64_t time_ms) {
...
int SequentailDecoder::__DecodeOnePacket() {
    ...
    if (is_slice_eof) { // current slice is eof,  read next slice
        if (in_slice_index_ + 1 < slices_.size()) {
            in_slice_index_ ++;
            if (!SequentailSlice::IsContinuous(slices_[in_slice_index_-1], slices_[in_slice_index_])) {
                if (slices_[in_slice_index_-1].IsSamePath(slices_[in_slice_index_])) {
                    in_display_pos_ = slices_[in_slice_index_].GetDisplayRannge()._start;
                    ret = demuxer_->Seek(slices_[in_slice_index_].DisplayTimeToSourceTime(in_display_pos_), 0);
                }
                else {
                    ret = __OpenSlice(in_slice_index_, slices_[in_slice_index_].GetDisplayRannge()._start)? 0 : -1;
                }
            }
        } else {
            is_all_eof = true;
...
//    int64_t target_time_ms = MAX(time_ms, slices_[index].GetDisplayRannge()._start);
    int64_t target_time_ms = slices_[index].GetTargetDisplayPosition(time_ms);
...
void SequentailDecoder::OnDecodedEnd() {
