...
#import "QEditorPlayer2.h"
#import "QCommon.h"
#import "QObserverHoster.h"
#import "QAudioPlayer2.h"
...
@interface QEditorPlayer2 ()
@property (nonatomic) QObserverHoster* observerHoster;
@property (nonatomic, readonly) dispatch_queue_t cbQueue;
@end

static QEditorPlayerState2 coreStateToXMState(PlayerState state)
{
    QEditorPlayerState2 xmState = XMPlayerState_Idle;
    switch (state) {
...
{
public:
    CallBackImpl(QEditorPlayer2* xmPlayer)
    {
        _xmPlayer = xmPlayer;
    }
    ...
    void onPlayerStateChanged(PlayerState newState, PlayerState oldState) override
    {
        dispatch_async(_xmPlayer.cbQueue, ^(){
            @autoreleasepool{
               QEditorPlayerState2 xmNewState = coreStateToXMState(newState);
               QEditorPlayerState2 xmOldState = coreStateToXMState(oldState);
...
private:
    __weak QEditorPlayer2* _xmPlayer;
};

#pragma mark - implementation XMEEPlayer

@implementation QEditorPlayer2 {
...
- (instancetype)initWithQueue:(dispatch_queue_t)cbQueue
{
    ...
    if ((self = [super initWithNative:_pPlayer.get()]) != nil) {
        ...
        //set audio target
        id<QAudioTarget> audioTarget = [[QAudioPlayer2 alloc] init];
